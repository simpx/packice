...
class NodeState:
    """Core lease engine shared by adapters."""

    def __init__(self, attachment_mode: str = "fs") -> None:
    ...
    def acquire(self, req: AcquirePayload) -> Dict:
        ...
        if req.intent == "create":
            ...
            if any(
                l["objid"] == req.objid and l["intent"] == "create" and self._active(l)
    ...
    def _grant_create(self, req: AcquirePayload, ttl: int) -> Dict:
        ...
        attachment = self._create_attachment(attachment_id)
    ...
    def _create_attachment(self, attachment_id: str) -> Attachment:
        now = self._now()
        if self.attachment_mode == "memfd":
            flags = getattr(os, "MFD_ALLOW_SEALING", 0)
            fd = os.memfd_create(f"packice-{attachment_id}", flags=flags)
            return {"fd": fd, "path": f"/proc/self/fd/{fd}", "sealed": False, "size": 0, "last": now, "mode": "memfd"}
        path = DATA_ROOT / f"{attachment_id}.bin"
        path.touch()
        return {"path": str(path), "sealed": False, "size": 0, "last": now, "mode": "fs"}

    def _attachment_size(self, attachment: Attachment) -> int:
        if attachment.get("mode") == "memfd":
            return os.fstat(attachment["fd"]).st_size  # type: ignore[arg-type]
        return Path(attachment["path"]).stat().st_size

    def _cleanup_attachment(self, attachment: Attachment) -> None:
        if attachment.get("mode") == "memfd":
            fd = attachment.get("fd")
            if fd is not None:
                try:
                    os.close(fd)
                except OSError:
...
__all__ = ["AcquirePayload", "LeasePayload", "NodeState", "DEFAULT_TTL", "DATA_ROOT"]
